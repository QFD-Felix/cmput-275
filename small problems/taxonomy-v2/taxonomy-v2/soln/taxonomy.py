...
def identify_thing(tree):
    ...
    if tree.get_children():
        ...
        (yes_tree, no_tree) = tree.get_children()
        ...
        if yes_answer:
            ...
            yes_tree = ValueTree.valshape_to_tree((new_name,[]))
            no_tree = ValueTree.valshape_to_tree((name,[]))
            tree.set_value(new_question)
            tree.set_children([yes_tree,no_tree])
...
def do_things(tree):
    def get_things(tree):
        ...
        if tree.get_children():
            (yes_tree, no_tree) = tree.get_children()
            return get_things(yes_tree) + get_things(no_tree)
        else:
