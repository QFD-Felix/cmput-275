...
int16_t srv_get_waypoints(LonLat32* waypoints,
    ...
    int16_t index;
    int32_t last_index;
    char lat_str[buf_size];
    char lon_str[buf_size];
    char buf[buf_size];
    int32_t lat;
    int32_t lon;
    ...
    for (int16_t i=0; i < path_len; ++i) {
        ...
        buf_len = serial_readline(buf, buf_size);
        Serial.println("A");
        index = string_read_field(buf, 2, lat_str, buf_len, " ");
        last_index = string_read_field(buf, index, lon_str, buf_len, " ");
