...
uint8_t is_waypoints_visible(int32_t x,int32_t y) {
      uint8_t r =
          screen_map_x < x &&
            x < screen_map_x + display_window_width &&
              screen_map_y < y &&
                y < screen_map_y + display_window_height;
            return !r;
        }
    ...
    if (select_button_event) {
        ...
        */
        ...
        if (request_state==RS_WAIT_FOR_START) {
            ...
            //Serial.println("R 53506319 -113500484 53505589 -113505174");
        ...
        if ( cur_path_len > 1 ) {
            ...
            */
            for(int i = 1; i<cur_path_len; i++){
              dprintf("in the for .......................");
              int32_t lcd_start_x = abs(longitude_to_x(current_map_num, waypoints[i-1].lon)-screen_map_x);
              int32_t lcd_start_y = abs(latitude_to_y(current_map_num,waypoints[i-1].lat)-screen_map_y);
              if (is_waypoints_visible(lcd_start_x,lcd_start_y)){
                int32_t lcd_end_x = abs(longitude_to_x(current_map_num, waypoints[i].lon)-screen_map_x);
                int32_t lcd_end_y = abs(latitude_to_y(current_map_num,waypoints[i].lat)-screen_map_y);
                dprintf(">>>>>>>>>>>>>>>%d %d %d %d>>>>>>>>>>", lcd_start_x,lcd_end_y,lcd_end_x,lcd_end_y);
                tft.drawLine(lcd_start_x,lcd_start_y,lcd_end_x,lcd_end_y,0x001F);
                dprintf("----------%d %d %d %d---------", lcd_start_x,lcd_end_y,lcd_end_x,lcd_end_y);
              }
            }
            }
